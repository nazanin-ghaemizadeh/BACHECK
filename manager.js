...
async function exportManagerDashboard(){
 ...
 const buffer=await wb.xlsx.writeBuffer(),blob=new Blob([buffer],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}),fileName=`management_dashboard_${window.englishFileToken?window.englishFileToken(state.metadata.model,"vehicle"):"vehicle"}.xlsx`;if(window.BAMCO_SAVE_BLOB)await window.BAMCO_SAVE_BLOB(blob,fileName);else{const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download=fileName;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),60000)}
}
