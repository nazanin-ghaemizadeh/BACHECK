(function(){
  ...
  function backup(){
    ...
    try{
      ...
      const fileName=`full_case_backup_${window.englishFileToken?window.englishFileToken(state.metadata.model,"vehicle"):"vehicle"}_${new Date().toISOString().slice(0,10)}.json`;
      if(window.BAMCO_SAVE_BLOB){window.BAMCO_SAVE_BLOB(blob,fileName).catch(err=>{console.error("Full-case backup save failed",err);alert(locale==="fa"?"ذخیره فایل پشتیبان ناموفق بود.":"The backup file could not be saved.")});}
      else{const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=fileName;a.style.display="none";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),60000);}
    }catch(err){
